...
export const AuthIcon = ({ name }: IconProps) => {
  ...
  switch (name) {
    case 'email': return <Mail color={color} size={size} />;
...
export const MutedIcon = ({ name }: { name: 'eye' | 'eye-off' }) => {
  const color = '#7d7f8c';
  const size = 20;

  switch (name) {
    case 'eye': return <Eye color={color} size={size} />;
    case 'eye-off': return <EyeOff color={color} size={size} />;
    default: return null;
  }
};

export const SocialIcon = ({ name }: { name: 'apple' | 'google' }) => {
  if (name === 'google') {
    return (
